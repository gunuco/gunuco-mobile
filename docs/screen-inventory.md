...
| D3 | Product Detail | Info, price, rating, wishlist, CTA | Lists / search / wishlist | GImage gallery, PriceDisplay, RatingView, WishlistButton, GButton, Review summary | Product detail | Unavailable overlay | Configure / Add, wishlist, see reviews | Product Options, Reviews, Cart |
| D4 | Product Options / Variants | Schema-driven options; add to cart | Product Detail / Cart edit | ProductOptionRenderer, QuantitySelector, AddOnSelector (if any), PriceDisplay, sticky CTA | Product options, quote/preview | Recalculating | Select options, Add to Cart | Cart |
